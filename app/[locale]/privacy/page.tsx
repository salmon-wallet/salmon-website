...
import { LegalBody, legalLastUpdated, privacyBlocks, type Locale } from '@/lib/legal-content';
...
export default async function PrivacyPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'metadata.privacy' });
  const loc = (locale as Locale) in privacyBlocks ? (locale as Locale) : 'en';
      ...
      <main className="pt-28 pb-20">
        <div className="mx-auto max-w-3xl px-6">
          ...
          <div className="prose prose-invert prose-sm max-w-none space-y-6 text-text-secondary leading-relaxed">
            ...
            <LegalBody blocks={privacyBlocks[loc]} locale={loc} />
          </div>
        </div>
      </main>
      <Footer />
